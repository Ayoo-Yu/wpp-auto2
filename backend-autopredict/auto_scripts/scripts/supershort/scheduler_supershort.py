...
# 任务状态
task_executed = False  # 废弃，保留以兼容旧代码
task_train_executed = False  # 记录当天训练任务是否成功执行
task_predict_executed = False  # 记录当天预测任务是否执行
...
def run_train():
    """执行训练脚本"""
    global task_train_executed
    
    # 检查今天的训练是否已完成
    today_date = datetime.now().strftime('%Y%m%d')
    train_done = is_train_done(today_date)
    print(train_done)
    logging.info(train_done)
    
    if train_done:
        logging.info(f"今天({today_date})的训练已执行过，无需重复执行")
        task_train_executed = True
        return
    
    if not task_train_executed:
        logging.info("执行 auto_train.py")
        
        # 使用绝对路径直接指向实际文件
        script_path = "/app/auto_scripts/scripts/supershort/auto_train.py"
        ...
        if exit_code == 0:
            logging.info("auto_train.py 执行成功")
            task_train_executed = True  # 标记当天训练任务已执行
        else:
            logging.error(f"auto_train.py 执行失败，退出码: {exit_code}")
...
def run_param_optimizer():
    ...
    if not param_opt_executed:
        ...
        # 使用绝对路径直接指向实际文件
        optimizer_path = "/app/auto_scripts/scripts/supershort/param_optimizer.py"
...
# 每天 2:00 执行训练任务
schedule.every().day.at("02:00").do(run_train)

# 每周六 1:00 执行参数优化任务
schedule.every().saturday.at("01:00").do(run_param_optimizer)

logging.info("定时任务启动成功，每周六 1:00 运行参数优化，每天 2:00 运行训练")
...
today_date = datetime.now().strftime('%Y%m%d')
train_done = is_train_done(today_date)
task_train_executed = train_done
param_opt_executed = is_param_opt_done()

logging.info(f"当前时间: {now.strftime('%Y-%m-%d %H:%M:%S')}, 星期: {weekday_names[current_weekday]}")
if train_done:
    logging.info(f"检测到今天({today_date})的训练已执行过，初始化状态为已完成")
logging.info(f"当前任务状态: 训练任务已执行={task_train_executed}, 参数优化任务已执行={param_opt_executed}")
...
while True:
    try:
        ...
        if current_weekday == 5 and current_hour >= 1: 
            # 每小时检查一次标志文件状态
            param_flag_exists = is_param_opt_done()
            if param_flag_exists:
                param_opt_executed = True
                
            if not param_opt_executed and not param_flag_exists:
                logging.warning("检测到周六 1:00 参数优化未执行，立即补救运行")
                run_param_optimizer()
            elif not param_opt_executed and param_flag_exists:
        ...
        if current_hour >= 2:
            ...
            if not task_train_executed and not train_done:
                logging.warning("检测到 2:00 训练未执行，立即补救运行 auto_train.py")
                run_train()
            elif not task_train_executed and train_done:
                logging.info("发现今天训练已执行过，更新任务状态")
                task_train_executed = True
