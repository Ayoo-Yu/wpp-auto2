...
def monitor_prediction(today_date):
    ...
    if not is_model_available(model_folder_today):
        error_msg = f"❌ 未有可用模型，无法执行预测。模型目录: {model_folder_today}"
        print(error_msg)
    ...
    if not os.path.exists(today_csv_dir):
        error_msg = f"❌ 预测文件目录不存在: {today_csv_dir}"
        print(error_msg)
        logging.error(error_msg)
        return False
    ...
    if not csv_files:
        error_msg = f"❌ 未找到任何预测CSV文件，目录: {today_csv_dir}"
        print(error_msg)
        logging.error(error_msg)
        return False
    ...
    for csv_filename in csv_files:
        ...
        try:
            ...
            error_msg = f"❌ 预测执行失败 {csv_filename}: {str(e)}"
            print(error_msg)
            logging.error(error_msg)
    ...
    if processed_files:
        ...
        error_msg = "❌ 没有处理任何预测文件，可能全部已处理或处理失败"
        print(error_msg)
...
def main():
    ...
    # 如果预测失败，返回非零退出码
    if not success:
        logging.error("预测任务失败，将返回非零退出码")
        print("❌ 预测任务失败，将返回非零退出码")
        sys.exit(1)
