...
from config import Today,MODEL_FOLDER
...
def train_and_evaluate(X_train, y_train, X_val, y_val, params_list, scaler, model_folder_today):
    ...
    scaler_filepath1 = os.path.join(save_path, scaler_filename)
    scaler_filepath2 = os.path.join(MODEL_FOLDER, scaler_filename)
    joblib.dump(scaler, scaler_filepath1)
    joblib.dump(scaler, scaler_filepath2)
    print(f"Scaler 已保存到 {scaler_filepath1}，并已经更新")
    
    # 如果找到最佳模型，则额外保存为 'model.joblib'
    if best_model is not None:
        best_model_filepath1 = os.path.join(save_path, 'model.joblib')
        best_model_filepath2 = os.path.join(MODEL_FOLDER, 'model.joblib')
        joblib.dump(best_model, best_model_filepath1)
        joblib.dump(best_model, best_model_filepath2)
        print(f"最佳模型 '{best_model_name}' 已保存到 {best_model_filepath1}，并已经更新")
    else:
        print("未找到最佳模型。")
