...
def preprocess_new_data(file_path, lags):
    ...
    # 预处理
    X= preprocess_data_pre(data)  # 不需要 y
...
def save_predictions_to_csv(predictions, output_dir,MODEL_PATH):
    """
    保存预测结果到 CSV 文件
    """
...
def predict(CSV_FILE_PATH, MODEL_PATH, SCALER_PATH, WINDOW_SIZE=16):
    ...
    print("预测完成。")
    # 保存预测结果
    predict_file_path = save_predictions_to_csv(predictions, output_dir,MODEL_PATH)
    print("预测结果已保存。")
